...
async function fetchAgenda(data) {
    ...
    try {
        const response = await fetch(`${apiUrl}?action=getSchedule&date=${data}`);
        if (!response.ok) {
...
async function handleBuscarReservas(matricula) {
    ...
    try {
        const response = await fetch(`${apiUrl}?action=getReservations&matricula=${matricula}`);
        const result = await response.json();
 ...
 */
async function handleCancelamentoReserva(rowId, slotId, atividade) {
...
document.addEventListener('DOMContentLoaded', () => {
    // Inicialização (Omitido para brevidade, mantido no código final)

    carregarAgenda();
    
    // Listeners de Data (Omitido para brevidade)
    
    // Fechar Modais (Omitido para brevidade)
    
    // Lógica de Login Admin (Omitido para brevidade)
    
    // Listeners de Adição (Omitido para brevidade)

    // Submissão do formulário de adição (Omitido para brevidade)
