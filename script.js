...
function fecharModal(modalElement) {
    if (modalElement) {
        ...
        // Adiciona a classe hidden após o término da transição
...
function getDayOfWeek(dataISO) {
    const data = new Date(dataISO + 'T00:00:00'); // Adiciona T00:00:00 para evitar fuso horário
...
function toggleAdminInputs() {
    ...
    if (isQuickMassage) {
        ...
        renderQuickMassageGrid(profissional, data);

        // Se for Quick Massage, o botão é habilitado/desabilitado por outra função (handleQuickMassageSelect)
        // Mas o estado inicial depende da data (já checado)
...
// ... restante do código (funções de renderização, submit, etc.) ...
